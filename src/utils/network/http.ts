...
import { baseUrl, timeout } from '@ebuild/presets';
...
const invalidMethods = ['get', 'GET', 'post', 'POST', 'put', 'PUT', 'delete', 'DELETE'] as const;
...
class Requestor {
    ...
    public async fetch<T = any>(
        ...
        const { request, requestKey } = await this.preFetch<T>(uri, method, params, options);
    ...
    private async onFetchResponse<T extends ResponseData>(response: Response, data: Ref<T | null>) {
