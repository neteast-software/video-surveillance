<template>
    <div class="relative w-full h-full">
        ...
        <div class="w-full h-full bg-black">
            ...
            ></video>
            <NSpin v-if="!videoIsReady || webRTCError" class="absolute-center" :description="webRTCError"></NSpin>
        </div>
        <ControlsBar
            v-if="controls"
            ref="controller"
        ...
        >
            <template #[slotName]="props" v-for="(_, slotName) in $slots" :key="slotName">
...
import { computed, ref, provide, unref, watch, onBeforeUnmount, onMounted } from 'vue';
import { useEventListener, useFullscreen, useNow, useDebounceFn } from '@vueuse/core';
import RecordRTC from 'recordrtc';
import { downloadFile } from '@ebuild/utils/other';
import { format, differenceInMilliseconds, endOfDay, startOfDay, addSeconds } from 'date-fns';
import { NSpin } from 'naive-ui';
import { getServerWebRTC, getRealtimeVideo, getRecordVideo, testRecordRatio } from '@ebuild/utils/network/api/security';
import { recordTimerKey, isReadyKey, isPlayingKey, isRecordKey, playSpeedKey } from './helper';
...
async function negotiate(ts: number) {
    ...
    try {
        if (isRecord.value) {
            ...
            data = await getRealtimeVideo(props.nvrId, props.channelId, btoa(pc!.localDescription!.sdp));
        }
    } catch (err) {
...
function startRecord() {
    ...
    recorder = new RecordRTC(mediaStream, { type: 'video', mimeType: 'video/mp4' });
