<template>
    <div class="bg-black/50 bar px-4 py-3">
        <section class="flex gap-x-18">
            ...
            ></RecordButton>
            <ControlButton :class="{ 'pointer-events-none': !isReady }" :icon="screenshot" @click="$emit('screenshot')"
                >截图</ControlButton
            >
        </section>
        <section class="flex items-center gap-x-18">
            ...
            <NPopover
                ...
                :content-style="{ backgroundColor: 'rgba(25, 25, 25, 0.9)', borderRadius: '10%', padding: '-5px' }"
        ...
        <section class="flex justify-end gap-x-18">
            ...
            <ControlButton :class="{ 'pointer-events-none': !isReady }" :icon="pictureInPicture" @click="$emit('pip')">
                画中画
            </ControlButton>
        </section>
        <VideoHistory
