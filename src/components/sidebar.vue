		...
		<div class="flex-col h-full">
			...
			>
				...
				<div class="text-3.5 flex-w-rest ml-2.5">
					{{ nickName }}
					<div class="text-#A0AEC0 text-12px">
						{{ companyNameAndJob }}
					</div>
				</div>
				...
				>
					<template #trigger>
						<div
							class="w-4 h-4 bg-white flex-center rounded-1 cursor-pointer"
						>
							<div
								class="i-icons:caretdown w-3 h-3 text-lightGrey"
							></div>
						</div>
					</template>
					<div
...
watch(route, (to) => {
	menuInstRef.value?.showOption(to.path);
});
