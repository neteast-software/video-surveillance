<template>
	<div
		...
		:style="{
			top: `${bubblePosition[1] - 490}px`,
			left: `${bubblePosition[0] - (isSmallScreen ? 270 : 310)}px`,
		}"
	>
		...
		>
			<header
				class="flex-y-center gap-3 mb-4 text-4.5 lt-laptop-(text-4 mb-3)"
			>
              ...
              curData.address
				...
				<div
					:class="
						curData.alarmInfo
							? 'mt-4 p-3'
							: 'px-3 max-h-0 opacity-0 m-0'
					"
					class="bg-#F4F8FF flex gap-3 lt-laptop-(p-2)"
				>
					<img
						:src="
							curData.img ||
					...
					/>
					<div
						class="text-#A0AEC0 list-desc w-60 lt-laptop-(w-55 text-3.5)"
					>
						{{
							curData.alarmInfo
								? curData.alarmInfo.title
								: "暂无预警信息"
						}}
					</div>
				</div>
