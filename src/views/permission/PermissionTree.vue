...
const nodeProps: TreeNodeProps = ({ option }) => {
  return {
    onContextmenu(e: MouseEvent): void {
      ...
      y.value = e.clientY;
      e.preventDefault();
      const children = (option[props.childrenField] || []) as TreeOption[];
...
function onContextMenuSelect(key: string) {
  console.log("onContextMenuSelect", key);
  let { checkedKeys } = props;
  if (key === "checkAll") {
    checkedKeys = [...new Set([...checkedKeys, ...childrenKeys])];
  } else {
    checkedKeys = checkedKeys.filter((item) => !childrenKeys.includes(item));
  }
